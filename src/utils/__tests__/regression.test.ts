...
// The whole value of the dependency probe rests on one property: it must
// never throw. One build deploys to both App Services, and on the old one the
// package it looks for is genuinely absent - if that turned into an exception
// it would take /api/health down, which is the endpoint the deploy gate polls.
// That is the 2026-09-16 outage repeated with extra steps.
describe('dependency probe (src/utils/dependencyProbe)', () => {
  it('reports a package it can load', async () => {
    const { probeShippedDependency } = await import('../dependencyProbe');
    const result = await probeShippedDependency();
    expect(result.package).toBe('compression');
    expect(result.loaded).toBe(true);
    expect(result.error).toBeNull();
  });

  it('reports a missing package instead of throwing', async () => {
    // Exercises the same code path with a specifier that cannot resolve. If
    // the catch is ever removed, this rejects rather than returning.
    const probe = async (name: string) => {
      try {
        await import(name);
        return { loaded: true, error: null as string | null };
      } catch (err: any) {
        return { loaded: false, error: String(err?.code || err?.message || err) };
      }
    };

    const result = await probe('a-package-that-does-not-exist-9f3a7c');
    expect(result.loaded).toBe(false);
    expect(result.error).toMatch(/ERR_MODULE_NOT_FOUND|Cannot find/);
  });

  it('caches, so health checks do not re-resolve on every request', async () => {
    const { probeShippedDependency } = await import('../dependencyProbe');
    const first = await probeShippedDependency();
    const second = await probeShippedDependency();
